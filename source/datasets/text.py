...
class TextDataset(object):
    ...
    def __init__(self, sample_filepath, skipgram_filepath, negative_samples=None, verbose=True):
        '''
        ...
        filepaths. 
         ...
         - negative_samples: The number of negaitive samples that were used to 
            generate the dataset. This is requried to calculate the tensor shapes
            when calling _deserialize_example 
    ...
    def _create_vocabulary(self, sample_fp):
        ...
        # Check if vocab dir already exists
        ...
        # Else create vocab and save to cache
            ...
            uniqe_tokens = self._unique_tokens(sample_fp)

            vocab_layer = tf.keras.layers.StringLookup(max_tokens=len(uniqe_tokens) + 1, output_mode='int')
            vocab_layer.adapt(np.array(list(uniqe_tokens)))            
    ...
    def _unique_tokens(self, sample_fp):
        '''
        This function takes a filepath to a walks dataset, then it 
        loads the walks in in chunks and extracts all the unique tokens 
        that occur. The unique tokens is the vocabulary
        '''

        walks = self._load_walks_as_iterator(sample_fp)
        unique_tokens = set()
        for walk in walks:
            data = walk.to_numpy()
    ...
    def _deserialize_example(self, record):
        ...
        assert self.negative_samples is not None, f'ERROR: negative sample shape is \'None\' have you passed negative sample argument to TextDataset constructor?'
        ...
        tensor_shape = (self.negative_samples + 1,)

        x = {

            'target': tf.reshape(tf.io.parse_tensor(example['target'], out_type=tf.int64), shape=(1,)),
            'context': tf.reshape(tf.io.parse_tensor(example['context'], out_type=tf.int64), shape=tensor_shape)

        }         
          

        y = tf.reshape(tf.io.parse_tensor(example['label'], out_type=tf.int64), shape=tensor_shape)
...
if __name__ == "__main__":
    ...
    td._create_vocabulary(walks_fp)

    print(td.text_vectorizer())
    # print(sorted(td.text_vectorizer().get_vocabulary()))
    print(len(td.text_vectorizer().get_vocabulary()))
